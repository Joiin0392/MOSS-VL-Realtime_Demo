...
from .pool import GatewayCapacityError
...
async def list_models(request: Request):
    pool = get_gateway_pool(request)
    url = pool.first_ready_url()
    if url is None:
        raise HTTPException(status_code=503, detail={
            "code": "no_available_replica", "message": "no READY sglang-omni replica"})

    def fetch() -> requests.Response:
        return requests.get(f"{url}/v1/models",
                            timeout=max(1.0, pool.s.sglang_omni_connect_timeout_s))
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail={
            "code": "bad_upstream", "message": "replica returned non-JSON /v1/models"})
