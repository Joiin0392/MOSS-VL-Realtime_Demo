...
class TokenIssuer:
    def __init__(self, ttl_s: float = 60.0):
        ...
        self._entries: Dict[str, Tuple[str, float]] = {}  # token → (session_id, expires_at)
        self._lock = threading.Lock()

    def mint(self, session_id: str, ttl_s: Optional[float] = None) -> str:
        token = secrets.token_urlsafe(32)
        expires = time.monotonic() + max(0.05, float(ttl_s if ttl_s is not None else self.ttl_s))
        with self._lock:
            self._entries[token] = (session_id, expires)
        return token
    ...
    def purge_expired(self) -> int:
        ...
        now = time.monotonic()
        with self._lock:
            stale = [t for t, (_, exp) in self._entries.items() if exp < now]
            for t in stale:
                del self._entries[t]
        if stale:
